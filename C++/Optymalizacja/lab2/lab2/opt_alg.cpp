...
solution HJ(matrix(*ff)(matrix, matrix, matrix), matrix x0, double s, double alpha, double epsilon, int Nmax, matrix ud1, matrix ud2)
{
    ...
    if (!HJ_steps.is_open()) {
        throw std::runtime_error("Nie mo¿na otworzyæ pliku do zapisu.");
    }
        ...
        while (true) {
            ...
            if (X.fit_fun(ff) < XB.fit_fun(ff)) {
                ...
                while (true) {
                    ...
                    double k1 = XB.x(0);  // Wartoœæ wspó³czynnika k1
                    double k2 = XB.x(1);  // Wartoœæ wspó³czynnika k2
                    HJ_steps << t << "; " << k1 << "; " << k2 << std::endl;  // Zapis wyników do pliku
                    t += 1;
                ...
                s *= alpha;  // Zmiana d³ugoœci kroku (jeœli potrzebna)
            }
...
solution Rosen(matrix(*ff)(matrix, matrix, matrix), matrix x0, matrix s0, double alpha, double beta, double epsilon, int Nmax, matrix ud1, matrix ud2)
{
	...
	try {
		...
		Rosen_steps.close(); // Zamkniêcie pliku przed powrotem
		return Xopt;
	}
	catch (const std::string& ex_info)
	{
		Rosen_steps.close(); // Upewnij siê, ¿e plik jest zamkniêty przy b³êdzie
