...
matrix mass_center(solution* Xopt, int dim, int exclude_index) {
	...
	// Zamiast rows() i cols(), uøyj dostÍpnych metod
	int rows = Xopt[0].x.rows();  // Zak≥adajπc, øe n_rows i n_cols istniejπ
	int cols = Xopt[0].x.cols();

	// Inicjalizuj macierz zerowπ o odpowiednich wymiarach
	matrix center = matrix(rows, cols, 0.0);

	// Iteruj po wszystkich wierzcho≥kach, pomijajπc wybrany indeks
	for (int i = 0; i <= dim; i++) {
		if (i == exclude_index) continue; // PomiÒ maksymalny wierzcho≥ek
		center = center + Xopt[i].x;     // Dodaj wspÛ≥rzÍdne kolejnych wierzcho≥kÛw
		count++;
	}

	// Wylicz úredniπ (úrodek masy)
	return center / static_cast<long double>(count);
}


double testowa_lab_3_wew_clear(double y, const matrix& x, const matrix& ud1, const matrix& ud2)
{
	return y + norm(x) + ud1(0, 0) + ud2(0, 0);
}
