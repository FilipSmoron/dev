...
{
	// Tworzymy wynikową macierz dy o rozmiarze 4x1
	matrix dy(4, 1);

	// Stałe
	long double C = 0.47;             // Współczynnik oporu
	long double ro = 1.2;             // Gęstość powietrza
	long double r = 0.12;             // Promień kuli
	long double S = PI * r * r;       // Powierzchnia przekroju poprzecznego
	...
	// Masa i przyspieszenie grawitacyjne
	long double m = 0.6;              // Masa obiektu
	long double g = 9.81;             // Przyspieszenie ziemskie

	// Równania różniczkowe
	dy(0) = y(1); // dx/dt = prędkość w osi x
	dy(1) = (-1) * (Fmx + Dx) / m; // d²x/dt² = przyspieszenie w osi x
	dy(2) = y(3); // dy/dt = prędkość w osi y
	dy(3) = (-1) * (Fmy + Dy + m * g) / m; // d²y/dt² = przyspieszenie w osi y

	// Zwracamy wynikową macierz dy
	return dy;
}
